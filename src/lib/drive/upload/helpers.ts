import { DEFAULT_MULTIPART_CHUNK_SIZE } from '@/lib/drive/types'
...
export function resolveTargetFolderID(input: TransferFileInput) {
  return input.folderId?.trim() || 'root'
}

export function resolveTargetFolderPath(input: TransferFileInput) {
...
export function resolveChunkSize(input: TransferFileInput) {
  const chunkSize = input.chunkSize ?? DEFAULT_MULTIPART_CHUNK_SIZE
  return Math.max(chunkSize, MIN_MULTIPART_PART_SIZE)
}

export function resolveConcurrency(input: TransferFileInput) {
