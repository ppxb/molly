...
import { getErrorMessage, updateFileRequest } from '@/lib/upload/client/api'
import type { UploadFolderRecord, UploadedFileRecord } from '@/lib/upload/shared'

import type { RenameTarget } from './types'

interface UseRenameEntryActionInput {
  currentFolderIdRef: MutableRefObject<string>
  loadEntries: (folderId: string) => Promise<void>
}

export function useRenameEntryAction({ currentFolderIdRef, loadEntries }: UseRenameEntryActionInput) {
