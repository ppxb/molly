...
import type { TaskSpeedTracker } from '@/features/upload/hooks/upload-queue/speed-tracker'
import type { UpdateTaskPatch } from '@/features/upload/hooks/upload-queue/task-patch'
import { getErrorMessage } from '@/lib/drive/client/api'
import { uploadFile } from '@/lib/drive/client/uploader'
import type {
  UploadNameConflictAction,
  UploadNameConflictPayload,
  UploadResumeState
} from '@/lib/drive/client/upload/types'
import type { UploadedFileRecord } from '@/lib/drive/shared'

export type AbortIntent = 'pause' | 'cancel'

interface UploadTaskRunnerOptions {
  taskID: string
  getTaskByID: (taskID: string) => UploadQueueTask | null
  patchTask: (taskID: string, patch: UpdateTaskPatch) => void
  enqueueNameConflict: (taskID: string, payload: UploadNameConflictPayload) => Promise<UploadNameConflictAction>
  ...
  onTaskFinalizeStartRef: MutableRefObject<((file: UploadedFileRecord) => Promise<void> | void) | undefined>
  onTaskFinalizeAbortRef: MutableRefObject<((file: UploadedFileRecord) => Promise<void> | void) | undefined>
  onTaskDoneRef: MutableRefObject<((file: UploadedFileRecord) => Promise<void> | void) | undefined>
