...
function toSafeTask(task: UploadQueueTask): UploadQueueTask | null {
...
export async function loadPersistedQueueTasks() {
  ...
  if (!db) {
    return [] as UploadQueueTask[]
  }

  try {
    const state = await new Promise<StoredUploadQueueState | null>((resolve, reject) => {
      const tx = db.transaction(STORE_NAME, 'readonly')
      const store = tx.objectStore(STORE_NAME)
      const request = store.get(STATE_KEY)
      request.onsuccess = () => resolve((request.result as StoredUploadQueueState | undefined) ?? null)
    ...
    const restoredTasks: UploadQueueTask[] = []
...
export async function persistQueueTasks(tasks: UploadQueueTask[]) {
  const db = await openDB()
  if (!db) {
    return
  }
  ...
  try {
    const safeTasks = tasks.map(toSafeTask).filter((task): task is UploadQueueTask => task !== null)
    ...
    const state: StoredUploadQueueState = {
      version: 1,
      tasks: safeTasks,
      savedAt: Date.now()
    }
