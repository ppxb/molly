...
import { buildTransferQueueOverview } from '@/features/upload/hooks/transfer-queue/overview'
import { pickQueuedTaskIDs } from '@/features/upload/hooks/transfer-queue/scheduling'
import { createTaskSpeedTracker } from '@/features/upload/hooks/transfer-queue/speed-tracker'
import { runUploadTask, type AbortIntent } from '@/features/upload/hooks/transfer-queue/task-runner'
import {
  buildPausedTaskState,
  type UpdateTaskPatch,
  applyTaskPatch
} from '@/features/upload/hooks/transfer-queue/task-patch'
import {
  buildQueuedTaskState,
  createTaskFingerprint,
  createTaskFromFile
} from '@/features/upload/hooks/transfer-queue/task-factory'
import { useLatestRef } from '@/features/upload/hooks/transfer-queue/use-latest-ref'
import { useNameConflictQueue } from '@/features/upload/hooks/transfer-queue/use-name-conflict-queue'
import { useQueuePersistence } from '@/features/upload/hooks/transfer-queue/use-queue-persistence'
import { uploadRuntimeConfig } from '@/lib/drive/runtime-config'
import { normalizeFolderPath } from '@/lib/drive/path'
import { DEFAULT_MULTIPART_CHUNK_SIZE, DEFAULT_MULTIPART_THRESHOLD } from '@/lib/drive/types'
...
export function useTransferQueue(options: UseTransferQueueOptions = {}) {
  const [tasks, setTasks] = useState<TransferQueueTask[]>([])
  ...
  const overview = useMemo(() => buildTransferQueueOverview(tasks, isQueueActive), [isQueueActive, tasks])
